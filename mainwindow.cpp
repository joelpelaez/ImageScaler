...
void MainWindow::updateList()
{
    ...
    ui->listWidget->clear();
    ui->listWidget->addItems(std::accumulate(scales.begin(), scales.end(), QStringList(), [](QStringList a, QSize b) {
                                 a.push_back(QString::number(b.width()) + "x" + QString::number(b.height()));
                                 return a;
                             }));
}
...
void MainWindow::on_actionSave_to_triggered()
{
    ...
    if (!scaler.exportToDirectory(dirName)) {
        ...
        message.setText(tr("Check if a image was loaded or output directory permissions."));
        message.setStandardButtons(QMessageBox::Ok);
        message.setIcon(QMessageBox::Warning);
        message.exec();
    }
